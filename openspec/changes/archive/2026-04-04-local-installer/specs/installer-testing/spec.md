...
Tests SHALL verify MCP config is removed on uninstall.
...
Tests SHALL verify skill file describes MCP tools, not `ows` CLI.

#### Scenario: Skill references MCP tools
- **WHEN** skill file is generated
- **THEN** contains `sign_message` tool description
- **AND** does NOT contain `ows sign` or `OWS_PASSPHRASE`

---

### Requirement: Integration test -- full happy path
End-to-end test: install with all 3 agents mocked, verify wallet, policy, key,
MCP config per agent, skill files, state file, output address.
